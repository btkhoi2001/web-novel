...
$("#send").click(async function(){
    if (!window.localStorage.getItem('token')) {
        ...
        if ( email == "" || content == "") {
            ...
            const token = window.localStorage.getItem('token');
            const res = await fetch(`http://localhost:5000/api/novel/${novelId}/chapter/${chapterId}/report`, {
                method: 'Post',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
...
const loadChapter = async () => {
    try {
        ...
        const res1 = await fetch(`http://localhost:5000/api/novel/${novelId}/chapter`);
        ...
        if (window.localStorage.getItem('token')) {
            ...
            let marked = bookmarks.filter((bookmark) => {
                return bookmark.novelId == novelId && bookmark.chapterId == chapterId;
            })[0];
            if(marked){
                $(".mark-up").toggleClass("d-none");
                $(".mark-down").toggleClass("d-none");
...
const displayChapter = (novel, chapter, listOfChapters) => {
    ...
    if(previousChapter) {
        $('.prev-chap').attr('href', `http://localhost:3000/novel/${novel.novelId}/chapter/${previousChapter.chapterId}`)
    }
    else {
    ...
    if(nextChapter) {
        $('.next-chap').attr('href', `http://localhost:3000/novel/${novel.novelId}/chapter/${nextChapter.chapterId}`)
    }
    else {
    ...
    htmlString4 = listOfChapters.map((chap) => {
        return `
