...
async function fetchNovel() {
    try {
        if(token) {
            ...
            const res2 = await fetch(`http://localhost:5000/api/novel/${novelId}/chapter`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
...
async function SetValueNovel(novel) {
    ...
    for (let i = substr.length - 1; i>=0; i--)
        if(substr[i] === `<`) {
            if(substr[i+1] !== `/`)
                check = substr.substring(i+1, i+1+substr.substring(i+1).indexOf(`>`))
            break;
        }
    let description
    if(novel.description.substring(850) === ``)
        ...
        if(check !== ``)
            description = substr + `</${check}>`+ temp + `<${check}>` + novel.description.substring(850) + last
        else
            description = substr + temp + novel.description.substring(850) + last
    }
    $("[itemprop=description]").html(description)
    if(token)
        $(".comment-list-item-cover-user").attr("src", User.user.avatar)
}
...
function nominating() {
    ...
    if(em)
        em.remove();
    const quantity = 1
    fetch(`http://localhost:5000/api/novel/${novelId}/nomination`, {
        method: 'POST',
        headers: {
        ...
        if(res.message){
            $('#suggest-book').append(`<h3 id="err-mess">${res.message}</h3>`)
            setTimeout(function() {
...
function filterNovels(authorId)
{
    ...
    for(let i = 0; i < listOfNovels.length; i++)
        if(listOfNovels[i].authorId === authorId)
            newL.push(listOfNovels[i])
